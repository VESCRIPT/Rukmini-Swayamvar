...
// Ensure ViewState includes all view options
type ExtendedViewState = ViewState | 'success-stories';
...
import { ChildSafetyPolicyComponent } from './child-safety-policy/child-safety-policy.component';
...
import { MarriageProfilePdfComponent } from './marriage-profile-pdf/marriage-profile-pdf.component';
import { BiodataMakerComponent } from './biodata-maker/biodata-maker.component';
import { CommunityHomeComponent } from './community/community-home/community-home.component';
import { CommunityEventsComponent } from './community/community-events/community-events.component';
import { EventDetailComponent } from './community/event-detail/event-detail.component';
import { TeachingsListComponent } from './community/teachings-list/teachings-list.component';
import { TeachingArticleComponent } from './community/teaching-article/teaching-article.component';
import { EventCategory, TeachingCategory } from './community/community.models';
import { ApiService } from './services/api.service';
import { ForgotPasswordComponent } from './forgot-password/forgot-password.component';
import { ReportedBlockedUsersComponent } from './reported-blocked-users/reported-blocked-users.component';
import { isDashboardSidebarView } from './core/constants/dashboard-sidebar-views';
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [
    ...
    MarriageProfilePdfComponent,
    BiodataMakerComponent,
...
export class AppComponent implements OnInit {
  ...
  previousView: string = 'home';
  selectedProfileId: any;
  selectedConversationId: string | null = null;
  selectedChatOtherUser: { name: string; avatar: string } | null = null;
  /** 'connected' = full chat; 'pending_sent' = request sent, allow one message; 'none' = show Connect to message */
  selectedConnectionStatus: 'connected' | 'pending_sent' | 'pending_received' | 'none' = 'connected';
  selectedEventId: string | null = null;
  selectedArticleId: string | null = null;
  selectedEventCategory: EventCategory | null = 'marriage';
  selectedTeachingCategory: TeachingCategory | null = null;
  selectedStep: number = 1;
  currentLang: 'en' | 'hi' | 'mr' = 'en';
  helpCenterDefaultSubView: 'faq' | 'create' | 'list' | 'detail' = 'faq';
  ...
  constructor(
    ...
    // Keep view state aligned with browser back/forward navigation.
    window.addEventListener('popstate', () => {
      this.zone.run(() => this.syncViewFromUrl());
    });
  }
  ...
  private translations: Record<'en' | 'hi' | 'mr', {
    ...
    deleteSuccessMessage: string;
      ...
      hi: {
        ...
        deleteAccountHeading: 'अपना खाता हटाएं',
        deleteSuccessMessage: 'खाता सफलतापूर्वक हटा दिया गया'
      },
      mr: {
        title: 'रुक्मिणी',
        subtitle: 'स्वयंवर',
        ...
        deleteAccountHeading: 'तुमचे खाते हटवा',
        deleteSuccessMessage: 'खाते यशस्वीरित्या हटवले'
      }
    };
  ...
  openProfileDetail(profileId: string | number) {
    this.selectedProfileId = profileId;
    this.setView('profile-detail');
  }

  openCommunityEvents(category: EventCategory | null): void {
    this.selectedEventCategory = category || 'marriage';
    this.setView('community-events');
  }
  ...
  openTeachings(category: TeachingCategory | null): void {
    this.selectedTeachingCategory = category;
    this.setView('teachings');
  }

  openTeachingArticle(articleId: string): void {
    this.selectedArticleId = articleId;
    this.setView('teaching-article');
  }

  get communityUsername(): string {
    try {
      const user = this.apiService.getUser();
      const firstName = user?.firstName || (user?.name && String(user.name).trim().split(/\s+/)[0]);
      const raw = (firstName && String(firstName).trim()) ? String(firstName).trim() : '';
      if (raw) {
  ...
  openChatDetail(payload: string | number | { otherUserId: string | number; conversationId?: string; name?: string; avatar?: string }) {
    const isObject = typeof payload === 'object' && payload !== null && 'otherUserId' in payload;
    const otherUserId = isObject ? (payload as any).otherUserId : payload;
    const conversationId = isObject ? (payload as any).conversationId : undefined;
    const name = isObject ? (payload as any).name : undefined;
    const avatar = isObject ? (payload as any).avatar : undefined;

    this.selectedProfileId = otherUserId;
    this.selectedConversationId = conversationId || null;
    this.selectedChatOtherUser = (name !== undefined || avatar !== undefined) ? { name: name ?? '', avatar: avatar ?? '' } : null;
    localStorage.removeItem('active_conversation_id');

    const currentUserId = this.resolveUserId();
    if (!currentUserId) {
      this.selectedConnectionStatus = 'connected';
      this.setView('chat-detail');
      return;
    }

    // Always verify connection status from API – never assume connected (so requests stay in Request tab until Accept)
    this.apiService.getConnectionStatus(currentUserId, String(otherUserId)).subscribe({
      next: (result) => {
        if (result.connected) {
          this.selectedConnectionStatus = 'connected';
          if (result.conversationId) {
            this.selectedConversationId = result.conversationId;
            localStorage.setItem('active_conversation_id', result.conversationId);
          } else if (conversationId) {
            this.selectedConversationId = conversationId;
            localStorage.setItem('active_conversation_id', conversationId);
          } else {
            this.apiService.createOrGetConversation({ userId: currentUserId, otherUserId: String(otherUserId) }).subscribe({
              next: (res) => {
                const convId = res?.data?.conversationId ?? res?.data?._id ?? res?.conversationId ?? res?._id;
                if (convId) {
                  this.selectedConversationId = String(convId);
                  localStorage.setItem('active_conversation_id', String(convId));
                }
                this.setView('chat-detail');
              },
              error: () => this.setView('chat-detail')
            });
            return;
          }
          this.setView('chat-detail');
        ...
        if (result.requestReceived) {
          this.selectedConnectionStatus = 'pending_received';
          if (conversationId) {
            this.selectedConversationId = conversationId;
            localStorage.setItem('active_conversation_id', conversationId);
          }
        ...
        if (result.requestSent) {
          this.selectedConnectionStatus = 'pending_sent';
          this.apiService.trackSentConnection({
            otherUserId: String(otherUserId),
            name: name || this.selectedChatOtherUser?.name || 'User',
            avatar: avatar || this.selectedChatOtherUser?.avatar || '',
        ...
        this.selectedConnectionStatus = 'none';
        if (conversationId) {
          this.selectedConversationId = conversationId;
          localStorage.setItem('active_conversation_id', conversationId);
        }
  ...
  onStepSelected(step: number) {
    this.selectedStep = Math.min(Math.max(1, step), 6);
    this.setView('profile-form');
  }

  // Map URL to View
  private syncViewFromUrl() {
    const path = this.location.path().replace('/', '') || 'home';
    ...
    const viewMap: Record<string, ExtendedViewState> = {
      ...
      'marriage-profile-pdf': 'marriage-profile-pdf',
      ...
      'teachings': 'teachings',
      'teaching-article': 'teaching-article'
    };

    let newView = viewMap[path] || 'home';
    if (!this.canAccessView(newView)) {
      if (path !== 'login') {
    ...
    if (this.currentView !== newView) {
      this.previousView = isDashboardSidebarView(newView) ? 'dashboard' : this.currentView;
      this.currentView = newView;
      window.scrollTo(0, 0);
    }
  }

  /** Views that require a logged-in session (JWT). */
  private readonly authRequiredViews: ExtendedViewState[] = [
    'welcome',
    'dashboard',
    'partner-preferences',
    'messages',
  ...
  private canAccessView(view: ExtendedViewState): boolean {
    if (!this.authRequiredViews.includes(view)) {
      return true;
    }
  ...
  // Update view and URL
  setView(view: ExtendedViewState) {
    ...
    if (view === 'help-support') {
      this.helpCenterDefaultSubView = 'list';
    } else if (view === 'help-center') {
      this.helpCenterDefaultSubView = 'faq';
    }

    if (fromView !== view) {
      this.previousView =
        isDashboardSidebarView(view) || fromView === 'dashboard'
          ? 'dashboard'
          : fromView;
    }
    ...
    const urlMap: Record<ExtendedViewState, string> = {
      'home': '/',
      'login': '/login',
      'register': '/register',
      'privacy-policy': '/privacy-policy',
    ...
    const shouldReplaceLoginHistory =
      view === 'login' &&
      fromView !== 'login' &&
      this.authRequiredViews.includes(fromView as ExtendedViewState) &&
      !this.apiService.isAuthenticated();

    this.router.navigateByUrl(newUrl, { replaceUrl: shouldReplaceLoginHistory });
    window.scrollTo(0, 0);
  }
